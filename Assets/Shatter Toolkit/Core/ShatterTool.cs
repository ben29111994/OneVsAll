        ...
        protected void CreateNewGameObjects(IList<IHull> newHulls, out GameObject[] newGameObjects)
        {
            ...
            for (int i = 0; i < newHulls.Count; i++)
            {
                ...
                if(gameObject.CompareTag("Bot"))
                    newGameObject.GetComponent<MeshRenderer>().material.color = Color.black;

                if(gameObject.CompareTag("BotGun"))
                ...
                if (rigidbody != null && newRigidbody != null)
                {
                    ...
                    newRigidbody.AddForce(new Vector3(Random.Range(-1000, 1000), 500, Random.Range(-1000, 1000)));
                ...
                Destroy(newGameObject.gameObject.GetComponent<BoxCollider>());
                newGameObject.AddComponent<BoxCollider>();
                Destroy(newGameObject.gameObject, 5);
                //newGameObject.transform.DOMove(newGameObject.transform.position, 2).OnComplete(() =>
                //{
                //    DOTween.To(() => newGameObject.GetComponent<Rigidbody>().velocity, x => newGameObject.GetComponent<Rigidbody>().velocity = x, new Vector3(0, 0, 0), 0.5f);
                //});
                ...
                if(newGameObjects[i].tag != "Obstacle")
                newGameObjects[i].tag = "Untagged";
            }
        }
    }
}
